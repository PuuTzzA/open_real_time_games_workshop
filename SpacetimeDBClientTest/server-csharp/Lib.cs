...
public static partial class Module
{
    ...
    [Table(Name = "color_change", Public = true)]
    public partial struct ColorChange
    {
        [PrimaryKey]
        public uint entity_id;
    ...
    [Reducer]
    public static void ChangeColor(ReducerContext ctx, uint colorChange)
    {
        Module.Player player = ctx.Db.player.identity.Find(ctx.Sender) ?? throw new Exception("Player not found");
        uint pid = player.player_id;

        var row = ctx.Db.color_change.entity_id.Find(pid);
        if (row == null)
        {
            ctx.Db.color_change.Insert(new ColorChange
            {
                entity_id = pid,
                color_id = colorChange,
            });
        }
        else
        {
            Module.ColorChange row_ = (Module.ColorChange)row;
            row_.color_id = colorChange;
            ctx.Db.color_change.entity_id.Update(row_);
        }
    }
