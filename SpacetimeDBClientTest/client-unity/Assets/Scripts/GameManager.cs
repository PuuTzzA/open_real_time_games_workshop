...
public class GameManager : MonoBehaviour
{
    ...
    void HandleConnect(DbConnection conn, Identity identity, string token)
    {
        ...
        conn.Db.ColorChange.OnInsert += ColorInsert;
        conn.Db.ColorChange.OnUpdate += ColorUpdate;
    ...
    public void Update()
    {
        ...
        {
            Debug.Log("lalalallalallalalalalalal");
            Conn.Reducers.ChangeColor(lol++);
        }
    }

    void ColorInsert(EventContext context, ColorChange colorChange)
    {
        Debug.Log($"Color change: {colorChange.EntityId} {colorChange.ColorId}");
        HandleColorChange(colorChange.EntityId, colorChange.ColorId);
    }

    void ColorUpdate(EventContext context, ColorChange oldColorChange, ColorChange newColorChange)
    {
        Debug.Log($"Color update: {newColorChange.EntityId} {newColorChange.ColorId}");
        HandleColorChange(newColorChange.EntityId, newColorChange.ColorId);
    }

    void HandleColorChange(uint id, uint colorId)
    {
        if (!Players.TryGetValue(id, out var entityController))
        {
            return;
        }
        entityController.ChangeColor(colorId);
    }
