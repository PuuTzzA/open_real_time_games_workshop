...
public class PlayerController : MonoBehaviour
{
    ...
    Sprite CreateWhiteTriangle()
    {
        Texture2D tex = new Texture2D(64, 64);
        Color32[] pixels = new Color32[64 * 64];
        for (int y = 0; y < 64; y++)
            for (int x = 0; x < 64; x++)
                if (x > 32 - y && x < 32 + y) pixels[y * 64 + x] = Color.white;
    ...
    public virtual void ChangeColor(uint id)
    {
        ...
        SetColor(colors[(int)id % colors.Count]);
    }
    ...
    public void Update()
    {
        ...
        child.transform.position = CenterOfMass() + new Vector2(1, 0) ?? (Vector2)child.transform.position;
